...
const authFile = path.join(__dirname, '../../../../playwright/.auth/prodUser.json');

setup('Production User', async ({ page }, testInfo) => {
  const loginPageFunctions = new LoginPageFunctions(page, testInfo);
  await loginPageFunctions.loginWithUserCredentials("production-validation");
